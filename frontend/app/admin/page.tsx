...
export default function Dashboard() {
  ...
  const runSweep = useCallback(async () => {
    ...
    setSweepMsg(null);
    try {
      const { run_id } = await triggerSweep(dry, fonte);
      pollRun(run_id);
    } catch (e) {
      if (e instanceof TriggerError && (e.code === 401 || e.code === 403)) {
        setSweepMsg("sessão expirada — recarregue a página pra logar de novo");
...
function ReversoSection() {
  ...
  const analisar = useCallback(async () => {
    ...
    try {
      const r = await chamar(url.trim());
      setRes(r);
      carregarHistorico();
    } catch (e) {
      if (e instanceof TriggerError && (e.code === 401 || e.code === 403)) {
        setErr("sessão expirada — recarregue a página pra logar de novo");
      } else {
